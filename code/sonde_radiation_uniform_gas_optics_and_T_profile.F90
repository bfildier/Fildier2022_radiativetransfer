...
program sonde_radiation
  ...
  real(wp), dimension(:,:), allocatable :: play, plev, tlay, \
                                           play_uniform, plev_uniform, tlay_uniform ! BF add
  ...
  type(ty_source_func_lw)     :: lw_sources_unused       ! BF: add dummy variable
  type(ty_optical_props_1scl) :: lw_optical_props_unused ! BF: add dummy variable
  type(ty_optical_props_2str) :: sw_optical_props_unused ! BF: add dummy variable
...
! BF add
  allocate(play_uniform(1,nlay),  tlay_uniform(1,nlay), plev_uniform(1,nlay+1))
  play_uniform(1,:) = read_field(ncid, "play_ref", nlay)
  tlay_uniform(1,:) = read_field(ncid, "tlay_ref", nlay)
  plev_uniform(1,:) = read_field(ncid, "plev", nlay+1)
  ...
  call stop_on_err(lw_sources_unused%alloc           (1, nlay, gas_optics_lw))  ! BF add
  call stop_on_err(lw_optical_props%alloc_1scl(1, nlay, gas_optics_lw))
  call stop_on_err(lw_optical_props_unused%alloc_1scl(1, nlay, gas_optics_lw))  ! BF add
...
!                                            lw_optical_props, lw_sources))
  ...
  ! BF: first compute uniform gas_optics
  call stop_on_err(gas_optics_lw%gas_optics(play_uniform, plev_uniform, tlay_uniform, &
                                            sfc_t, gas_concs, &
                                            lw_optical_props, lw_sources_unused))
  
  ! BF: then compute uniform source term
  call stop_on_err(gas_optics_lw%gas_optics(play, plev, tlay_uniform, &
                                            sfc_t, gas_concs, &
                                            lw_optical_props_unused, lw_sources))
